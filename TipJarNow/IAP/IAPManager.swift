...
final class IAPManager {
    ...
    /// ASC API V1 GET /v1/apps/6770249058/inAppPurchasesV2 confirms:
    ///   productID = com.jiejuefuyou.tipjarnow.premium
    static let premiumProductID = "com.jiejuefuyou.tipjarnow.premium"
    ...
    /// Apple Review precedent (AutoChoice round-4 v1.0.5 2026-05-11): the
    /// reviewer reported that `purchase()` outcomes were silently swallowed
    /// for `.userCancelled` / `.pending` / `@unknown default` / `.unverified`,
    /// with only thrown errors surfacing via a buried `lastError` string.
    /// This explicit state machine drives a visible, actionable UI on every
    /// branch so reviewers can verify purchase outcomes without ambiguity.
    ...
    private static let cachedIsPremiumKey = "TipJarNow.iap.cachedIsPremium"
